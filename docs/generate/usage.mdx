---
id: usage
title: "Simplify code generation"
description: "Unlock the simplicity of Buf Generate: A step-by-step guide to revolutionize your development process"
---

> We highly recommend completing [the tour](../tutorials/getting-started-with-buf-cli.md#generate-code) to get an
> introduction to `buf generate`.
...
### 5.1 Generating from a Buf module {#from-module}
...
### 5.2 Generating using multiple templates {#multiple-templates}
...
### 5.3 Generating to a specific directory {#output}
...
In short, Protobuf development can be difficult, but the buf generate command simplifies the process by allowing
developers to focus on schema definitions rather than complex protoc commands. By creating a buf.gen.yaml file and
running buf generate, developers can quickly and efficiently generate code. BSR's remote plugins also offer a solution
to issues caused by differences in compiler versions and custom Protobuf plugins.

## Next steps

For more information on Buf Generate, check out our [manual](/generate/overview). To understand more about remote
plugins, check out our [explanation](/bsr/remote-plugins/overview).
