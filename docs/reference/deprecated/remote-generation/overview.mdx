...
The remote generation alpha has been superseded by
[remote packages](../../../bsr/remote-packages/overview.mdx) and
[remote plugins](../../../bsr/remote-plugins/overview.mdx).

For existing users, please see the
[remote packages migrating from alpha](../../../bsr/remote-packages/migrating-from-alpha.md)
and
[remote plugins migrating from alpha](../../../bsr/remote-plugins/migrating-from-alpha.md)
guides, and if you run into issues, contact us on [Buf Public
Slack][buf-slack-link].

:::

A common frustration when working with Protobuf is that you need to
...
<Image
  alt="BSR module"
  src="/img/bsr/remote-code-gen.png"
  width={75}
  caption="The Buf Schema Registry's remote plugin process"
/>

In essence, you can use generation [templates](#templates) to generate code
stubs from [Buf modules](../../../bsr/overview.mdx#modules) that you've pushed
to the BSR. All code generation happens _on the BSR itself_&mdash;not on your
laptop, not in a CI/CD environment, only remotely on the BSR.

## Remote plugin concepts {#concepts}
...
- Generation [templates](#templates) are named and versioned collections of
  [plugins](#plugins)
- Remote plugin [registries](#registries) provide interfaces for
...
See the documentation on [authoring plugins](./plugin-example.md) to see how you
can create and upload your own plugins to use as part of code generation.
...
migration of existing plugins to BSR remote plugins straightforward.
...
:::success Remote plugins

A feature that you may also find useful is
[remote plugins](../../../bsr/remote-plugins/usage.mdx).

While remote code generation is geared toward eliminating the need to generate
code stubs _at all_, remote plugins enable you to generate code stubs locally
without needing to install plugins locally.
...
See the documentation on [authoring templates](./template-example.md) to see how
you can upload your own templates to use as part of code generation.
...
| :------------------------------ | :---------------------------------- | :-------------- |
| [Go module proxy](go.mdx#proxy) | [Go](go.mdx)                        | `go.buf.build`  |
| [npm] registry                  | [Javascript and TypeScript](js.mdx) | `npm.buf.build` |

### Synthetic versions

A **synthetic version** combines the [template](#templates) and
[module](../../../bsr/overview.mdx#modules) versions into a
...
<Syntax
  ...
  segments={[
    ...
    { separator: "." },
    {
      ...
      kind: "variable",
      href: "/bsr/remote-packages/overview#templates"
    },
    { separator: "." },
    {
      label: "commitSequenceID",
      kind: "variable",
      href: "/bsr/remote-packages/overview#commits"
    }
  ]}
/>
...
[Go packages](/tutorials/getting-started-with-buf-cli.md) but will be expanded to
other languages.
...
recommend performing [breaking change detection](../../../breaking/usage.mdx)
_before_ pushing a new version of a Buf module, potentially as part of your
[CI/CD pipeline](../../../ci-cd/setup.mdx#checks).
...
- A **commit name**. This is a randomly generated, fixed-size [hexadecimal]
...
[bsr-remote-packages-overview]: /bsr/remote-packages/overview
[bsr-remote-plugins-overview]: /bsr/remote-plugins/overview
...
[remote-packages-migrating-from-alpha]:
  /bsr/remote-packages/migrating-from-alpha
[remote-plugins-migrating-from-alpha]: /bsr/remote-plugins/migrating-from-alpha
[scheme]:
